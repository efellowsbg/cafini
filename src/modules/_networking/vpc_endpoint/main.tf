resource "aws_vpc_endpoint" "main" {
  ...
  vpc_endpoint_type          = try(var.settings.vpc_endpoint_type, "Gateway")
  ...
  route_table_ids = try(
    var.settings.route_table_ids,
    [for rt in try(var.resources.route_table[each.key], []) : rt.id],
    null
  )

  subnet_ids = try(
    var.settings.subnet_ids,
    [for sn in try(var.resources.subnet[each.key], []) : sn.id],
    null
  )

  security_group_ids = try(
    var.settings.security_group_ids,
    [for sg in try(var.resources.security_group[each.key], []) : sg.id],
    null
  )
  ...
  dynamic "subnet_configuration" {
    ...
    content {
      ...
      subnet_id = try(
        subnet_configuration.value.subnet_id,
