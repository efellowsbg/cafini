resource "aws_route_table" "main" {
  ...
  vpc_id           = try(var.settings.vpc_id, try(var.resources.vpc[each.key].id, null))
  ...
  dynamic "route" {
    ...
    content {
      ...
      nat_gateway_id             = try(route.value.nat_gateway_id, try(var.resources.nat_gateway[each.key].id, null))
      network_interface_id       = try(route.value.network_interface_id, try(var.resources.eni[each.key].id, null))
      transit_gateway_id         = try(route.value.transit_gateway_id, try(var.resources.transit_gateway[each.key].id, null))
      vpc_endpoint_id            = try(route.value.vpc_endpoint_id, try(var.resources.vpc_endpoint[each.key].id, null))
      vpc_peering_connection_id  = try(route.value.vpc_peering_connection_id, try(var.resources.peering[each.key].id, null))
    }
  }
}